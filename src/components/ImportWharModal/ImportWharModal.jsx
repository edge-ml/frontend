  ...
  return (
    <Modal isOpen={isOpen} onClose={running ? closeBackground : dismiss} size="lg">
      ...
      <ModalBody>
        ...
        {job && (
          <div className="p-2">
            ...
            {failed && (
              <Alert color="red">{status.error || "Import failed"}</Alert>
            )}
          </div>
        )}
      </ModalBody>
      <ModalFooter>
        {!job && (
          <>
            <Button variant="default" onClick={dismiss} className="me-2">
              Cancel
            </Button>
            <Button disabled={!selected} onClick={startImport}>
              Import
            </Button>
          </>
        )}
        {job && running && (
          <Button variant="default" onClick={closeBackground}>
