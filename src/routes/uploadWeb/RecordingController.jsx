import SimpleEventEmitter from "../../services/SimpleEventEmitter";
import {
  createDataset,
  appendToDataset,
...
export class RecordingController extends SimpleEventEmitter {
  constructor(sensors, sampleRates, datasetName, uploadInterval = UPDATE_INTERVAL) {
    super();
  ...
  _upload = async (series) => {
    ...
    let data = Object.entries(series).map(([name, dataPoints]) => ({
      name,
      data: dataPoints,
    }));
    data = data.map((elm) => {
  ...
  async start() {
    ...
    this.newDataset = await createDataset({
    ...
    const datasetId = this.newDataset?.id ?? this.newDataset?._id;
    if (!datasetId) {
      throw new Error("Failed to create dataset for recording");
    }
