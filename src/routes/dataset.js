...
class DatasetPage extends Component {
  ...
  labelingClickHandler(e) {
    ...
    function plotbandClickHandler(e) {
      const line = State.datasetPage.chart.xAxis.plotLinesAndBands.filter(
        elem => elem.moving === true
      )[0];
      if (line !== undefined) {
        return;
      }
      if (
        ...
        this.id === State.datasetPage.edit.selectedBand.id
      ) {
        State.datasetPage.edit.selectedBand = -1;
      } else {
    ...
    function lineMoveHandler(e) {
      ...
      const band = State.datasetPage.chart.xAxis.plotLinesAndBands.filter(
    ...
    const pos = e.point.x;
    const bands = this.xAxis.plotLinesAndBands.filter(
    ...
    const initialColor =
      State.datasetPage.states[State.datasetPage.currentIndex].color;
  ...
  componentDidMount() {
    ...
    load('/protocol/Dataset.proto')
      ...
      .then(res => {
        ...
        maxdate.setSeconds(
          maxdate.getSeconds() + State.datasetPage.defaultRange
        );
    ...
    function mouseMoveHandler(original, e) {
      ...
      if (e.type === 'mousemove') {
        ...
        const newBand = State.datasetPage.chart.xAxis.plotLinesAndBands.filter(
          elem => elem.id === `band_${id}`
        )[0];
  ...
  componentWillUnmount() {
    if (
      typeof State.datasetPage.chart !== undefined &&
  ...
  render() {
    return (
      <Loader loading={!this.state.chart.ready}>
        <DatasetToolbar />
        <Row>
          <Col onMouseUp={this.mouseUpHandler}>
            <HighchartsReact
        ...
        <Button outline color="success" onClick={this.gencsv}>
          <a>
            <CloudDownloadIcon className="svg-green" /> Download CSV
          </a>
        </Button>
      </Loader>
    );
