...
function applyTo(io, socket) {
    ...
    socket.on('labelings_labels', () => {
        ...
        Promise.all([
            rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_DEFINITIONS)),
            rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_TYPES))
        ])
    ...
    socket.on('add_labeling', newLabeling => {
        ...
        rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.POST, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_DEFINITIONS, newLabeling))
        .then(() => rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_DEFINITIONS))
            .then(labelings => {
    ...
    socket.on('update_labeling', labeling => {
        ...
        rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.PUT, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_DEFINITIONS + `/${labeling['_id']}`, labeling))
        .then(() => rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_DEFINITIONS)))
        .then(labelings => {
            socket.emit('err', false);
            io.emit('labelings_labels', {labelings, labels: undefined});
    ...
    socket.on('delete_labeling', labelingId => {
        if (!socket.client.twoFactorAuthenticated) return;  //TODO: see above
        rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.DELETE, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_DEFINITIONS + `/${labelingId}`))
        .then(() => {
            Promise.all([
                rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_DEFINITIONS)),
                rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_TYPES))
    ...
    socket.on('update_labeling_labels', (labeling, labels, deletedLabels) => {
        ...
        Promise.all(
            ...
            .map(label => rp(generateRequest(socket, CONSTANTS.HTTP_METHODS.POST, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.LABEL_TYPES, label))))
            .then(newLabels => {
