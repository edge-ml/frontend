...
function applyTo(io, socket) {
    ...
    socket.on('experiments', () => {
        ...
        rp(generateRequest(CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.EXPERIMENTS))
    ...
    socket.on('add_experiment', newExperiment => {
        ...
        rp(generateRequest(CONSTANTS.HTTP_METHODS.POST, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.EXPERIMENTS, newExperiment))
        .then(() => rp(generateRequest(CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.EXPERIMENTS))
            .then(experiments => {
    ...
    socket.on('update_experiment', experiment => {
        ...
        rp(generateRequest(CONSTANTS.HTTP_METHODS.PUT, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.EXPERIMENTS + `/${experiment['_id']}`, experiment))
        .then(() => rp(generateRequest(CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.EXPERIMENTS)))
        .then(experiments => {
            socket.emit('err', false);
            io.emit('experiments', experiments);
    ...
    socket.on('delete_experiment', experimentId => {
        ...
        rp(generateRequest(CONSTANTS.HTTP_METHODS.DELETE, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.EXPERIMENTS + `/${experimentId}`))
        .then(() => {
            rp(generateRequest(CONSTANTS.HTTP_METHODS.GET, CONSTANTS.API_URI, CONSTANTS.ENDPOINTS.EXPERIMENTS))
            .then(experiments => socket.emit('experiments', experiments))
        })
